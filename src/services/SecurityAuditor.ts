...
export class SecurityAuditor {
  ...
  private readonly HEADERS_TO_AUDIT = [
   ...
   */
  private _auditHeaders(response: Response): HeaderAuditResult[] {
    const receivedHeaders = response.headers();
    const auditResults: HeaderAuditResult[] = [];
   ...
   */
  private async _detectMixedContent(page: Page): Promise<MixedContentResult[]> {
    const insecureRequests: MixedContentResult[] = [];
